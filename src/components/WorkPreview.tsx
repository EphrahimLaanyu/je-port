...
const PROJECTS = [
  {
    ...
    title: "Deejay Kace",
    category: "Music",
    // Referenced directly from public/assets
    img: "/assets/Screenshot 2026-01-29 102644.png", 
    tagline: "Redefining digital liquidity.",
    link: "http://deejaykace.co.ke/"
  },
  {
    id: "02",
    title: "NFCCO",
    category: "Conservancy",
    img: "/assets/WhatsApp Image 2026-01-29 at 10.14.14.jpeg",
...
const WorkPreview = () => {
  ...
  const handleMouseEnter = (index: number) => {
    // Only trigger hover effect on non-touch devices
    ...
    >
      {PROJECTS.map((project, index) => (
        <div
          ...
          style={{ 
            flexGrow: (typeof window !== 'undefined' && window.innerWidth > 768) ? (hoveredIndex === index ? 6 : hoveredIndex === null ? 1 : 2) : 1,
          }}
        >
          {/* IMAGE LAYER */}
          <div className="absolute inset-0 w-full h-full overflow-hidden">
            <Image 
              ...
              alt={project.title} 
              fill
              // Since we are using fill, sizes is important for performance
              sizes="(max-width: 768px) 100vw, 33vw"
              className="object-cover grayscale md:grayscale transition-all duration-[2000ms] ease-out md:group-hover:scale-110 md:group-hover:grayscale-0"
              style={{ filter: (typeof window !== 'undefined' && window.innerWidth < 768) ? 'grayscale(0%)' : undefined }}
            />
          ...
          <div className="relative w-full h-full flex items-end md:items-center justify-start md:justify-center p-8 md:px-12 pb-12 md:pb-0">
            ...
            <div className={`content-${index} flex flex-col items-start max-w-2xl transition-opacity duration-300 
                ${(typeof window !== 'undefined' && window.innerWidth < 768) ? 'opacity-100' : (hoveredIndex === index ? 'opacity-100' : 'opacity-0 pointer-events-none')}
            `}>
