...
const Navbar = () => {
  ...
  const getSectionData = () => {
    switch (pathname) {
      case '/work': return { num: "02 / 05", name: "Work" };
      case '/about': return { num: "03 / 05", name: "About" };
      case '/contact': return { num: "04 / 05", name: "Contact" };
      case '/process': return { num: "05 / 05", name: "Contact" };
      default: return { num: "01 / 05", name: "Home" };
    }
  };
  ...
  return (
    <>
        ...
        <nav ref={navRef} className="relative flex items-center justify-between pointer-events-auto transition-all">
          <Link href="/" className="group flex items-center gap-4 cursor-pointer">
            <div className="flex items-center -space-x-2 transition-transform duration-500 group-hover:scale-110">
                {/* Place your logo in /public/logo-burgundy.png */}
                <Image src="/assets/logo-burgundy.png" alt="L" width={36} height={36} className="w-7 md:w-9 opacity-90" />
                <Image src="/assets/logo-burgundy.png" alt="R" width={36} height={36} className="w-7 md:w-9 opacity-90 scale-x-[-1]" />
            </div>
          ...
          <div className="flex items-center gap-6 md:gap-10">
            <Link href="/contact" className="group relative hidden sm:flex items-center justify-center px-8 py-2 border border-[#4a0404]/30 rounded-full bg-white/20 overflow-hidden">
                ...
                <div className="absolute inset-0 bg-[#4a0404] translate-y-full group-hover:translate-y-0 transition-transform duration-700 ease-in-out"></div>
            </Link>
      ...
      <div 
        ref={menuOverlayRef} 
        className="fixed inset-0 bg-[#4a0404] z-[2000] flex flex-col justify-between p-8 md:p-16 overflow-hidden" 
        style={{ clipPath: "circle(0% at 90% 10%)" }}
      >
        <div className="absolute inset-0 opacity-[0.04] pointer-events-none bg-[url('https://grainy-gradients.vercel.app/noise.svg')]" />
        
        <div className="relative z-10 flex justify-between items-start">
          <div className="menu-detail flex flex-col text-[#EAE8E4]">
            <span className="font-mono text-[9px] text-[#EAE8E4]/30 uppercase tracking-[0.5em]">Navigation</span>
            <span className="font-serif italic text-lg">Maison J&E</span>
            <span className="font-mono text-[10px] mt-1 opacity-50">{localTime} NBO</span>
          </div>
          <button onClick={() => setIsMenuOpen(false)} className="group flex items-center gap-4">
            ...
            <div className="w-12 h-12 border border-[#EAE8E4]/10 rounded-full flex items-center justify-center group-hover:bg-[#EAE8E4] group-hover:text-[#4a0404] transition-all duration-500 text-xl font-light text-[#EAE8E4]">×</div>
          </button>
        </div>
            ...
            <div key={i} className="portal-button w-full max-w-lg">
              ...
              >
                <div className="flex items-baseline gap-6">
                  <span className="font-mono text-[10px] opacity-30 group-hover:text-[#EAE8E4] group-hover:opacity-100 transition-all text-[#EAE8E4]">{item.num}</span>
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-end gap-12 border-t border-[#EAE8E4]/05 pt-8">
          ...
          <div className="menu-detail text-right hidden md:block">
            ...
            <p className="font-serif italic text-[#EAE8E4]/80">Nairobi, Kenya — Worldwide</p>
          </div>
        </div>
      </div>
    </>
